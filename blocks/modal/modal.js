...
export async function createModal(contentNodes, id) {
    ...
    contentNodes = Array.from(contentNodes).filter((node) => node.nodeType === 1);
    const noCloseButton = contentNodes.some((node) => node.classList.contains('no-close-button'));
    const fullWidth = contentNodes.some((node) => node.classList.contains('full-width'));
